...
//< Statements and State parse-conditional
  ...
  private Expr comma() {
      ...
      while (match(COMMA)) { // assumes you have a COMMA token type
          Expr right = assignment();
          expr = new Expr.Comma(expr, right); // left-associative
      }
...
//> term
  private Expr term() {
    // ERROR PRODUCTION: operator with no left operand
    if (match(PLUS, MINUS)) {
        error(previous(), "Missing left-hand operand.");
        factor(); // parse and discard right operand
        return new Expr.Literal(null); // dummy expression
    }
    Expr expr = factor();
